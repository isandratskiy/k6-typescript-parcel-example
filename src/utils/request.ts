...
export function post<T>(_url: string, body: any, headers: any, interceptor?: K6Interceptor): T {
  const url = `${BASE_URL}/${_url}`
  const response: RefinedResponse<'text'> = http.post(url, JSON.stringify(body), headers)

  return responseAs<T>(response, url, interceptor)
}

export function get<T>(_url: string, headers: any, interceptor?: K6Interceptor): T {
  const url = `${BASE_URL}/${_url}`
...
export function responseAs<T>(res: RefinedResponse<'text'>, url: string, interceptor?: K6Interceptor): T {
